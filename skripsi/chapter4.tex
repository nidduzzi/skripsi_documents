...
After this, in the third step, the solution functions \(u \) are generated with 25 modes in time and 25 in space and 40 unique function samples for each viscosity value. In total 120 unique solution functions are generated. To ensure the same solutions are generated every time, the random number generator uses a new instance of the generator with the same seed number. This results in the solutions being the same for all viscosity and other parameters of the equation are adapted to this.
...
\begin{figure}[H]
  ...
  \begin{subfigure}{\linewidth}
    \begin{adjustbox}{width=\linewidth}
      \input{figures/exact_antiderivative_label.pgf}
    \end{adjustbox}
    \caption{}\label{fig:exact_antiderivative_label}
  \end{subfigure}
  \begin{subfigure}{\linewidth}
    \begin{adjustbox}{width=\linewidth}
      \input{figures/exact_antiderivative_prediction.pgf}
    \end{adjustbox}
...
\noindent The second scenario results are presented in two parts which are the coefficient predictions and function value predictions. The functions are first preprocessed in order to format them into current and next values in time as discussed in \lccref{sec:data_transformation}. The coefficient value predictions are shown in \lccref{table:scenario_2_spectral_metrics}. The RBF kernel parameter computed from the data is once again the same for all versions of the dataset with a value of 10. The testing metrics show that the performance of the model vary with the viscosity value \(\nu \). Starting with the inviscid Burgers' equation, with \(\nu=0\), the model performs relatively well and close to the best result with \(\nu=0.01\). The function value evaluated from the predictions is shown in \lccref{table:scenario_2_function_metrics}. The metrics across the board shows that the function value relative metrics are worse compared to the coefficients. Comparing the absolute metrics for the same table, we see the same general trend that the higher viscosity show the model performing worse.

\begin{table}[H]
  \caption{Performance metrics of coefficient prediction of next time step in scenario 2 by noise level.}\label{table:scenario_2_spectral_metrics}
  \centering
  \begin{tabular}{lrrrrr}
    \toprule
    \(\nu\) & MSE  & RMSE & MAE  & R2   & sMAPE \\
    \midrule
    0.0     & 0.01 & 0.09 & 0.06 & 0.87 & 0.60  \\
    0.01    & 0.01 & 0.08 & 0.06 & 0.90 & 0.56  \\
    0.1     & 0.03 & 0.17 & 0.13 & 0.49 & 1.09  \\
    \bottomrule
  \end{tabular}
\end{table}

\begin{table}[H]
  \caption{Performance metrics of function values evaluated from coefficient prediction of next time step in scenario 2 by noise level.}\label{table:scenario_2_function_metrics}
  \centering
  \begin{tabular}{lrrrrr}
    \toprule
    \(\nu\) & MSE      & RMSE & MAE  & R2   & sMAPE \\
    \midrule
    0.0     & 6.36e-04 & 0.03 & 0.02 & 0.85 & 0.66  \\
    0.01    & 5.19e-04 & 0.02 & 0.02 & 0.88 & 0.62  \\
    0.1     & 2.28e-03 & 0.05 & 0.04 & 0.45 & 1.13  \\
    \bottomrule
  \end{tabular}
\end{table}

The correlation image and p-matrix for each viscosity can be seen in \lccref{fig:scenario_2_interpretation}. Analyzing the correlation images first, we see that some columns are sorted in correlation to how the samples are sorted based on the real component of the 4th output wave number. For the inviscid equation, this correlation is the strongest with multiple columns exhibiting the correlation. The highest and visually most sorted values are from the forcing term portion of the inputs. A weaker correlation is still present of the viscosity of \(\nu=0.01\). However, once we get to the highest viscosity case, we see a seemingly random arrangement of the correlation image values. This observation and the lower scores can both be explained by the level of temporal discretization. This is the same issue we encounter when we use some traditional methods for higher viscosity values with the same time step for lower viscosity values \autocite{kassamFourthOrderTimeSteppingStiff2005,seydaogluNumericalSolutionBurgers2016}. The issue arises in stiff differential equations. This term describes the presence of a rapidly varying component. We can see that the larger amplitudes of high frequency components in the forcing term to represent the rapidly varying components as shown in \lccrefs{fig:burgers_forcing_0.0,fig:burgers_forcing_0.01,fig:burgers_forcing_0.1}. Traditionally, stiff equations would require specific solvers purpose built to efficiently solve the problem. Otherwise, as with our method, the solving process would require much finer time steps.
\begin{figure}[H]
  \centering
  \begin{subfigure}{0.49\linewidth}
    \begin{adjustbox}{width=\linewidth}
