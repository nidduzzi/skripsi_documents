...
The Fourier series can be represented in two ways, the first is the real representation by using sine and cosines as the basis functions in the series shown in \lccref{eq:fourier_series_sine_cosine}. The second representation is the complex representation which uses Euler's formula as the basis functions shown in \lccref{eq:fourier_series_complex}. Both of these are equivalent and are just the different representations. The relationship between the two representations is defined in \lccref{eq:fourier_coefficient_equivalency}.
\begin{align}
  f(x)=       & \hat{a}_0 + \sum_{k=0}^{\infty} \hat{a}_k\cos(2\pi kx) + \hat{b}_k\sin(2\pi kx)\label{eq:fourier_series_sine_cosine} \\
  f(x)=       & \sum_{k=-\infty}^{\infty} \hat{c}_k e^{2\pi ikx}\label{eq:fourier_series_complex}                                    \\
...
\begin{align}
  \dv{x}f(x)=  & \sum_{k=-N/2}^{N/2-1} \hat{c}_k (2\pi ik) e^{2\pi ikx} \label{eq:fourier_series_complex_derivative} \\
  =            & \sum_{k=-N/2}^{N/2-1} \hat{f'}_k e^{2\pi ikx}\nonumber                                              \\
  \hat{f'}_k = & (2\pi ik)\hat{c}_k\label{eq:fourier_coeff_derivative}
\end{align}
...
\begin{enumerate}
  ...
  \item R\textsuperscript{2}
        ...
        \begin{equation}
          r2 = 1 - \frac{\sum_{i=1}^{n}(y_{i} - y'_{i})^{2}}{\sum_{i=1}^{n}(y_{i} - \bar{y})^{2}}\label{eq:r2}
        \end{equation}
  ...
  \item sMAPE
        ...
        \begin{equation}
          sMAPE = \frac{2}{N} \sum_{i=1}^{N}\frac{\abs{e_{i}}}{\max(\abs{y_{i}} + \abs{y'_{i}},\epsilon)}
        \end{equation}
