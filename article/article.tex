...
\begin{frontmatter}
  ...
  \author[ku]{Hadi Susanto}
  \ead{hadi.susanto@ku.ac.ae}
  \author[upi]{Lala Septem Riza}
  \ead{lala.s.riza@upi.edu}

  \affiliation[upi]{organization={Department of Computer Science Universitas Pendidikan Indonesia},%Department and Organization
    addressline={Jl. Dr.\ Setiabudi No.229},
    postcode={40154},
    city={Bandung},
    country={Indonesia}}
  \affiliation[ku]{organization={Department of Mathematics Khalifa University},%Department and Organization
    addressline={Shakhbout Bin Sultan St - Hadbat Al Za`Faranah},
    postcode={127788},
    city={Abu Dhabi},
...
The training process is shown in \lccref{fig:spectralsvr_training}. The process in psuedocode is also shown in \lccref{alg:spectralsvr_training_psuedocode}. The process starts with determining the parameter and solution functions. Typically, the parameter functions would include the forcing term and others such as the initial solution function values. The solution function typically is the state variable that the PDE constraints using its derivatives, such as the displacement of a membrane for the wave equation. Next, the features and labels are constructed. Generally, the Fourier coefficients for both functions are computed using the DFT\@. And then, the parameter and solution coefficients are used as the feature and labels respectively. To finalize the construction, the coefficients are flattened into one dimension for each sample. And then, the features are scaled for training the LSSVR\@. Finally, after the feature and label construction, we train the LSSVR by computing the parameters of the model. In the psuedocode this starts by computing the kernel matrix from the features. And then the two matrices, \(\vb{A}\) and \(\vb{B}\), are put together to compute the model parameters via least-squares, which marks the end of the training process.
\begin{algorithm}[hbp]
  \caption{SpectralSVR Training Psuedocode.}\label{alg:spectralsvr_training_psuedocode}
  \begin{algorithmic}[1]
    \Procedure{TrainSpectralSVR}{$\vb{U}, \vb{F}, C $}
    \State{\(\vb{\hat{U}} \gets DFT(\vb{U})\)}
    \State{\(\vb{\hat{F}} \gets DFT(\vb{F})\)}
    \Comment{Compute Fourier coefficients of training functions}
    \State{\(\vb{X} \gets finalizeFeatures(\vb{\hat{U}},\vb{\hat{F}})\)}
    \State{\(\vb{y} \gets finalizeLabels(\vb{\hat{U}},\vb{\hat{F}})\)}
    \State{\(\Omega \gets K(\vb{X})\)}
    \Comment{Construct matrix of inner products}
    \State{\(\vb{A} \gets [[]]\)}
    \State{\(\vb{B} \gets [[]]\)}
    \State{\(\vb{A}[1:,1:] \gets \Omega+\frac{\vb{I}}{C}, \vb{A}[0,:] \gets 0,\vb{A}[:,0] \gets 0\)}
    \State{\(\vb{B}[0] \gets 0, \vb{B}[1:] \gets \vb{y}\)}
    \State{\(\vb{S} \gets leastSquares(\vb{A},\vb{B})\)}
    \State{\(\vb{\alpha} \gets \vb{S}[1:],\vb{b} \gets \vb{S}[0]\)}
    \Comment{Learned model parameters}
    \State{\textbf{return} \(\vb{\alpha}, \vb{b}\)}
    \EndProcedure{}
  \end{algorithmic}
\end{algorithm}

% TODO: add figures of how a dataset of 1D functions is represented and the transformed coefficient representation
% The data transformations are visualized in \lccref{fig:spectralsvr_training_data}.
% \begin{figure}[hbp]
%   \centering
%   \tikzfig{figures/training_spectralsvr_data}
%   \caption{SpectralSVR training data transformations.}\label{fig:spectralsvr_training_data}
% \end{figure}
